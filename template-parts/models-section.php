<?php
/**
 * Template part for displaying industry models grid on homepage
 */
require_once get_stylesheet_directory() . '/data.php';
$all_models = get_semi_models();
$homepage_models = array_slice($all_models, 0, 4);

$category_map = [
    'AI Infrastructure' => [
        'accent' => 'accent-primary',
        'icon'   => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" /><path d="M8 21h8M12 17v4" stroke-linecap="round" /></svg>'
    ],
    'Cloud Computing' => [
        'accent' => 'accent-secondary',
        'icon'   => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6.5 19a4.5 4.5 0 0 1-.9-8.9 6 6 0 0 1 11.8 0A4.5 4.5 0 0 1 17.5 19" stroke-linecap="round" stroke-linejoin="round" /><path d="M12 19v-7M9 15l3-3 3 3" stroke-linecap="round" stroke-linejoin="round" /></svg>'
    ],
    'Semiconductors' => [
        'accent' => 'accent-primary',
        'icon'   => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="5" width="14" height="14" rx="1" /><path d="M9 5v14M15 5v14M5 9h14M5 15h14" stroke-linecap="round" /></svg>'
    ],
    'Data Centers' => [
        'accent' => 'accent-primary',
        'icon'   => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="8" rx="2" ry="2" /><path d="M6 6h.01M6 18h.01" stroke-linecap="round" stroke-linejoin="round" /></svg>'
    ]
];
?>
    ...
    <div class="container" style="max-width:1100px;">
            ...
            <div class="flex flex-col items-start lg:items-end gap-3 lg:text-right">
                ...
                <a href="<?php echo esc_url( home_url( '/models' ) ); ?>"
                   ...
                   style="text-decoration:none;">
                    Explore all models <span class="group-hover:translate-x-1 transition-transform duration-200 inline-block">→</span>
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <?php $stagger = 1; foreach ( $homepage_models as $model ) :
                $cat = $model['category'];
                $config = isset($category_map[$cat]) ? $category_map[$cat] : [
                    'accent' => 'accent-primary',
                    'icon'   => '<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>'
                ];
                $accent = $config['accent'];
            ?>
            <a href="<?php echo esc_url( home_url( '/models/' . $model['slug'] ) ); ?>"
               ...
               style="text-decoration:none;">
                <div class="flex flex-col bg-root rounded-xl border border-border-strong h-full p-5 relative overflow-hidden transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
                    ...
                    <!-- Description -->
                    <p class="text-content-secondary leading-relaxed text-[12px] mb-4 flex-grow line-clamp-2">
                        <?php echo esc_html( $model['description'] ); ?>
                    </p>

                    <!-- Use case -->
                    <?php if ( !empty( $model['useCase'] ) ) : ?>
                    <div class="mb-4 px-3 py-2 bg-surface rounded-lg border border-border-subtle/40">
                        ...
                        <p class="text-[11px] font-medium text-content-secondary leading-normal">
                            <?php echo esc_html( $model['useCase'] ); ?>
                        </p>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-border-subtle mt-auto">
                        <div>
                            ...
                            <p class="text-[12px] font-bold text-<?php echo $accent; ?>"><?php echo esc_html( $model['dataPoints'] ); ?></p>
                        </div>
                        <div class="text-right">
                            <p class="text-[9px] uppercase tracking-widest text-content-tertiary font-bold">Cadence</p>
                            <p class="text-[12px] font-bold text-content-primary">Quarterly</p>
                        </div>
                        <span class="text-[11px] font-bold text-<?php echo $accent; ?> group-hover:translate-x-0.5 transition-transform duration-200 inline-block">
                            Request Access →
                        </span>
                    </div>
                </div>
